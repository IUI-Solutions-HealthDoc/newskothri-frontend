...
export async function buildNewsArticleJsonLd(id: string): Promise<Record<string, unknown> | null> {
  ...
  if (authorName) {
    ...
    const avatarPath = article.author?.avatar ? String(article.author.avatar).trim() : "";
    if (avatarPath) {
      const authorImage = toAbsoluteUrl(avatarPath);
      if (authorImage) authorObj.image = authorImage;
    }
